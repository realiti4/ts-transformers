...
class ChronosConfig:
    ...
    vocab_size: int = 4096  # number of tokens: 50,000 BPE merges + 256 bytes tokens + 1 <|endoftext|> token
...
class T5Attention(nn.Module):
    def __init__(self, config: ChronosConfig, has_relative_attention_bias=False):
        super().__init__()
    ...
    def compute_bias(self, query_length, key_length, device=None):
        ...
        values = self.relative_attention_bias(relative_position_bucket)  # shape (query_length, key_length, n_head)
        values = values.permute([2, 0, 1]).unsqueeze(0)  # shape (1, n_head, query_length, key_length)
        return values
...
class Block(nn.Module):
    def __init__(self, config, has_relative_attention_bias=False) -> None:
        super().__init__()
...
class Stack(nn.Module):
    def __init__(self, config: ChronosConfig, embed_tokens=None):
        super().__init__()
        ...
        self.block = nn.ModuleList(
            [Block(config, has_relative_attention_bias=bool(i == 0)) for i in range(config.n_layer)]
        )
    ...
    def forward(
        ...
        cross_attn_head_mask = self.get_head_mask(cross_attn_head_mask, self.config.n_layer)
...
class T5Model(nn.Module):
    def __init__(self, config: ChronosConfig):
        ...
        self.encoder = Stack(encoder_config, self.shared)

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        decoder_config.is_encoder_decoder = False
        decoder_config.n_layer = config.n_decoder_layer
        self.decoder = Stack(decoder_config, self.shared)
    ...
    def from_pretrained(cls, model_type):
        ...
        assert model_type in {"amazon/chronos-t5-tiny"}
        from transformers import T5ForConditionalGeneration

        print("loading weights from pretrained gpt: %s" % model_type)

        # num_layer, n_head and n_embd are determined from model_type
        ...
        model_hf = T5ForConditionalGeneration.from_pretrained(model_type)
        sd_hf = model_hf.state_dict()

        model_size = sum(p.numel() for p in model_hf.parameters()) / 1_000_000
    ...
    def forward(
        ...
        if head_mask is not None and decoder_head_mask is None:
            if self.config.n_layer == self.config.num_decoder_layers:
                print("warning")
